...
def resize_polgon(poly, scale, img):
    poly_new = poly.copy()
    centroid = np.mean(poly_new, axis=0)

    # scale the points
    poly_new = ((poly_new - centroid) * scale + centroid).astype(int)
...
while frame_idx < len(input_frames)-2: 
    ...
    # for nseg in range(1, num_segs+1):
    #     resized_mask = np.zeros_like(curr_seg)
    #     resized_seg_pts = resize_polgon(seg_pts[nseg], scale, curr_frame)
    # resized_mask = np.zeros_like(curr_seg)
    # resized_seg_pts = resize_polgon(seg_pts, scale, curr_frame)
    # resized_mask = cv.fillPoly(mask, [np.flip(resized_seg_pts, axis=2)], 255)
    # curr_kps, curr_descs = orb.detectAndCompute(curr_frame, reszied_mask)
    ious_gt = []
    ious = []
    ...
    for nseg in range(1, num_segs+1):
        ...
        n_seg_pts = seg_pts[nseg]
        resized_mask = np.zeros_like(curr_seg)
        resized_seg_pts = resize_polgon(n_seg_pts, scale, curr_frame)
        resized_mask = cv.fillPoly(resized_mask, [np.flip(resized_seg_pts, axis=2)], 255)
        curr_kps, curr_descs = orb.detectAndCompute(curr_frame, resized_mask) 
        ...
        seg_curr_kps = curr_kps
        seg_curr_descs = curr_descs
        ...
        try: 
            breakpoint()
            matrix, _ = cv.findHomography(u_pts, v_pts, cv.LMEDS)
            print(matrix)
        ...
        scale = np.abs(delta_vel/seg_pts[nseg].mean(axis=0)).mean()*10 + 1.0
