...
class MultiObjectTrackingNode:
    ...
    def seg_callback(self, msg):
        ...
        self.curr_seg = np.frombuffer(msg.data, dtype=np.uint8).reshape(data.height, data.width, -1)
    ...
    def input_callback(self, msg):
        ...
        if self.seg_updated:
            ...
            self.curr_frame = np.frombuffer(msg.data, dtype=np.uint8).reshape(data.height, data.width, -1)
