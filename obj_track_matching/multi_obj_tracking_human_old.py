...
data_path = "/Users/catherineglossop"
gt_path = os.path.join(data_path, "video")
...
for img in sorted(os.listdir(os.path.join(gt_test_path))):
    ...
    viz_combined_frames.append(frame[..., 0]*(255//len(np.max(frame))))

num_segs = np.max(np.asarray(combined_frames))
